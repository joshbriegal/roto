...
class PeriodFinder(ABC):
    ...
    def plot_periodogram(self, ax: Axes, period: PeriodResult, colour: Optional[str] = "orange") -> Axes:
        ...
        ax.set_xlim([max(np.diff(self.timeseries).min(), period_axis.min()), self.timeseries.max() - self.timeseries.min()])
